...
{
	// https://www.shadertoy.com/view/fsXczN Voxel Hall Colors, by elsif
	// Code has been modified a bit to fit library constraints
    /*
    Voxel Hall Colors

    Forked from Shane's Voxel Corridor.
    I removed the texture and added a surface color with a time gradient.
...
// of Fabrice Neyret's "ouside the box" thinking. :)
    //mat2 rot2(simdfloat a) { vec2 v = sin(vec2(1.570796, 0) + a);	return mat2(v, -v.y, v.x); }
    mat2 rot2(simdfloat a) { vec2 v = sin(vec2(1.570796, 0) + a);	return mat2(v.x, v.y, -v.y, v.x); }
    ...
    vec2 path(simdfloat z) {
        //return vec2(0); // Straight.
        simdfloat a = sin(z * 0.11);
        simdfloat b = cos(z * 0.14);
    ...
    simdfloat map(vec3 p){
         ...
         simdfloat n = dot(sin(p*1. + sin(p.yzx*.5 + iTime*.0)), vec3(.25)); // Sinusoidal layer.
    ...
    */
    ...
    simdfloat map(vec3 p) {
        ...
        simdfloat n = 5. - length(p.xy() * vec2(1, .8));

        // Square tunnel. Almost redundant in a voxel renderer. :)
        //n = 4. - max(abs(p.x), abs(p.y)); 

        // Tunnel with a floor.
        return min(p.y + 3.f, n); //n = min(-abs(p.y) + 3., n);

    }

    
    simdfloat brickShade(vec2 p){
    ...
    //
    const simdfloat w2h = 2.; // Width to height ratio.
    const simdfloat mortW = 0.05; // Morter width.

    simdfloat brickMorter(vec2 p) {
        ...
        return smoothstep(0.f, mortW, p.x) * smoothstep(0.f, mortW * w2h, p.y);

    }

    simdfloat brick(vec2 p) {
    ...
    simdfloat bumpSurf3D(vec3 p, vec3 n) {
       ...
       // else if (n.y > 0.5) p.xy(p.zx());
		...
		simdmask nxMask = n.x > 0.5;
		simdmask nyMask = n.x <= 0.5 && n.y > 0.5;

        vec2 temp;
        temp = blendv(temp, p.zy(), nxMask);
        temp = blendv(temp, p.zx(), nyMask);
        p.xy(temp);
    ...
    vec3 doBumpMap(vec3 p, vec3 nor, simdfloat bumpfactor) {

        const vec2 e = vec2(0.001, 0);
        simdfloat ref = bumpSurf3D(p, nor);
    ...
    /*vec3 doBumpMap(sampler2D tx, vec3 p, vec3 n, simdfloat bf) {
    ...
    //
    vec3 voxelTrace(vec3 ro, vec3 rd, vec3& mask) {
        ...
        for (int i = 0; i < 64; i++) {
            ...
            mask = blendv(step(side, side.yzx()) * (1. - step(side.zxy(), side)), mask, hit);
            side = blendv(side + mask * dRd, side, hit);
            p = blendv(p + mask * rd, p,  hit);
        }
    ...
    simdfloat calcVoxAO(vec3 vp, vec3 sp, vec3 rd, vec3 mask) {
    ...
    vec4 mainImage(vec2 fragCoord) {
        ...
        lightPos.xy(lightPos.xy() + path(lightPos.z));

        // Using the above to produce the unit ray-direction vector.
        simdfloat FOV = PI / 2.; // FOV - Field of view.
        ...
        simdfloat t = max(max(tCube.x, tCube.y), tCube.z);


        // Initialize the scene color.
        vec3 sceneCol = vec3(0);

		simdmask maskFar = t < FAR; 
       ...
       // if (t < FAR) 
           if (!maskNone(maskFar))
           {
            ...
            // Ambient occlusion.
            simdfloat ao = calcVoxAO(vPos, sp, rd, mask);//calculateAO(sp, sn);//*.75 + .25;
            ...
            simdfloat lDist = max(length(ld), 0.001f);

            // Normalize the light direction vectors.
            ld /= lDist;

            // Light attenuation, based on the distances above.
            simdfloat atten = 1. / (1. + lDist * .2 + lDist * 0.1); // + distlpsp*distlpsp*0.025

            // Ambient light.
            simdfloat ambience = 0.25;

            // Diffuse lighting.
            simdfloat diff = max(dot(sn, ld), 0.0f);

            // Specular lighting.
            simdfloat spec = pow(max(dot(reflect(-ld, sn), -rd), 0.0f), 32.f);
            ...
            vec3 texCol = tint + step(abs(snNoBump.y), .5f);
